...
```bash
# 1. Скачать последний бандл (подставьте нужную версию, например vX.Y.Z)
VERSION=vX.Y.Z
curl -LO "https://github.com/DanilenkA/awg-portal/releases/download/${VERSION}/awg-portal-${VERSION}-bundle.tar.gz"

# 2. Распаковать
mkdir awg-portal && cd awg-portal
tar xzf "../awg-portal-${VERSION}-bundle.tar.gz"
...
# Docker-образ (включает amneziawg-go, теги: latest + версия из git describe)
make build-docker
